          ...
          <section class="list-wrapper custom-scroll scroll">
            <custom-table :thead="thead" :tbody="tbody" :scroll="true">
              <template slot="item" slot-scope="props">
                ...
                <td><div>{{props.item.endTime}}</div></td>
                <td><div>{{props.item.orderNo}}</div></td>
...
const thead = ['开始时间', '结束时间', '合同编号', '关联机床编号', '客户名称', '派工服务类型']
...
export default {
  ...
  mounted () {
    this.params3.boxNo = sessionStorage.getItem('mtNo')
    this.selectBoxSendList()
  },
  methods: {
    jumpTo () {
      this.pageChange()
    ...
    // 获取关联历史
    selectBoxSendList () {
      this.$store.dispatch('a:contract/selectBoxSendList', {parameters: {...this.params3}, ...this.pageInfoReq}).then(
        res => {
          this.tbody = res.list || []
          // 把pageInfo封装成对象
          if (res) {
